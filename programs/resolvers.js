const resolvers = {
  ...
  Program: {
    async partners(program, _args, context, _info) {
  ...
  Query: {
    async programs(_parent, args, context, _info) {
      const { Program } = context.models;
    ...
    async program(_parent, args, context, _info) {
      const { Program } = context.models;
